...
while luot_hien_tai < so_luot_toi_da:
    ...
    so_doan = int(
        input(
            f"Luot {luot_hien_tai}/{so_luot_toi_da} - "
            "Nhap so ban doan (1-100): "
        )
    )
    if so_doan == so_can_doan:
        print(
            f"Chinh xac! Ban da doan dung "
            f"sau {luot_hien_tai} luot."
        )
        break
    ...
    print(
        f"Ban da het luot doan. "
        f"So can tim la: {so_can_doan}"
    )